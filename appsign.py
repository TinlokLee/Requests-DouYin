...
class DouYin(object):
	def __init__(self, width = 500, height = 300):
		...
		self.headers = {
			...
			'user-agent': 'Mozilla/5.0 (Linux; U; Android 5.1.1; zh-cn; MI 4S Build/LMY47V) AppleWebKit/537.36 \
                (KHTML, like Gecko) Version/4.0 Chrome/53.0.2785.146 Mobile Safari/537.36 XiaoMi/MiuiBrowser/9.1.3',
			'X-Real-IP': str(rip),
	...
	def get_video_urls(self, user_id, type_flag='f'):
		...
		try:
			process = Popen(['node', 'byte_crawle.js', str(user_id)], stdout=PIPE, stderr=PIPE)
	...
	def run(self):
		...
		user_id = input('请输入ID (例如111111):')
		user_id = user_id if user_id else '111111'
